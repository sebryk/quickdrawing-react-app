...
export class AuthService {
   ...
   async createPinterestAuthorizationUrl() {
      ...
      const authUrl = new URL(this.configService.get<string>('PINTEREST_AUTH_URL', 'https://www.pinterest.com/oauth/'))
      ...
      authUrl.searchParams.set('scope', this.configService.get<string>('PINTEREST_SCOPE', 'user_accounts:read'))
   ...
   async handlePinterestCallback({ code, state, response }: CallbackPayload) {
      ...
      response.cookie(this.configService.get<string>('SESSION_COOKIE_NAME', 'qd_session'), sessionToken, {
         httpOnly: true,
      ...
      response.redirect(this.configService.get<string>('FRONTEND_URL', 'http://localhost:3000'))
   }

   private async requestPinterestTokens(code: string) {
      const tokenUrl = this.configService.get<string>('PINTEREST_TOKEN_URL', 'https://api.pinterest.com/v5/oauth/token')
   ...
   private async getPinterestProfile(accessToken: string) {
      const userUrl = this.configService.get<string>('PINTEREST_USER_URL', 'https://api.pinterest.com/v5/user_account')
   ...
   // Used later for token checks / refresh flows.
